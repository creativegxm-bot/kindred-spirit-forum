...
import { Star, Download, Brain, ArrowRight } from "lucide-react";
import { Button } from "@/components/ui/button";

const TrendingAIApps = () => {
  const { navigate, localePath } = useLocalizedNavigate();
  const { language } = useLanguage();

  const topApps = [...aiApps]
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-bold text-foreground">
            {language === "tr" ? "Trend AI Uygulamaları" : "Trending AI Apps"}
          </h2>
        </div>
        <a href={localePath("/ai-apps")}>
